...
/**
 * Pure client-side component handling the search, category filter, and post grid.
 * Extracted from berita/page.tsx so the parent can be a Server Component
 * and export `metadata` for proper SEO.
 */
export default function BeritaClient() {
  ...
  return (
    <>
      {/* Dynamic page header */}
      <header className="bg-primary-700 text-white py-10 px-4">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold mb-2">{t('berita', 'title')}</h1>
      ...
      {posts.length > 0 && !loading && (
        <div className="flex justify-center mt-10">
          <button id="berita-load-more" className="btn-outline px-8">
            {loadMore}
          </button>
        </div>
      )}
